...
{
  blogs.map((blog,index)=>  <div key={index} data-aos="fade-zoom-in"
  ...
  data-aos-offset="0"  className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
   <div className="lg:h-96 lg:w-72">
     <img className="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125" src={blog.photo} alt="" />
   </div>
   ...
   <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
     ...
     <p className="mb-3 text-lg  text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">{blog.title}</p>
<button  onClick={()=>document.getElementById('my_modal_4').showModal()} className="btn relative inline-flex items-center justify-start overflow-hidden transition-all bg-[#E05EA6] hover:bg-white group px-8 py-2 rounded-full">
 ...
 <span className="w-full  text-white transition-colors duration-300 ease-in-out group-hover:text-white z-10">
  ...
  <div className="modal-box w-11/12 max-w-5xl">
    <h3 className="font-bold text-lg text-[#E05EA6]">বিস্তারিত জানুন.........</h3>
    <p className="p-4 text-justify space-x-2 ">{blog.description}</p>
